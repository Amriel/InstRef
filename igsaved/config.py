...
class Config:
    ...
    # Перед імпортом читати, що РЕАЛЬНО лежить у бібліотеці Eagle. Локальна
    # база памʼятає лише те, що відправляли ми: після перевстановлення, скидання
    # історії чи ручного імпорту вона впевнено каже «цього там немає» — і в
    # бібліотеці зʼявляється другий такий самий елемент. Знімок бібліотеки —
    # єдина чесна відповідь на питання «чи це вже там».
    eagle_check_library: bool = True
    # Описувати ВСЮ бібліотеку Eagle, а не лише теку застосунку. Бібліотека
    # референсів збирається не тільки цим застосунком: у ній лежить і те, що
    # притягнули з Pinterest чи кинули руками, — і опис із тегами потрібен
    # усьому, бо шукають по всій бібліотеці, а не по теці.
    describe_whole_library: bool = False
    ...
    # Модель на кілька гігабайт відеопамʼяті не потрібна між проходами, а
    # LM Studio тримає завантажене, доки не скажуть інакше. Після проходу
    # застосунок просить її вивантажити; ttl страхує на випадок падіння —
    # LM Studio сама звільнить памʼять через стільки секунд простою (0 — не
    # надсилати ttl узагалі, для серверів, які його не розуміють).
    vision_unload_after_run: bool = True
    vision_ttl_seconds: int = 900
